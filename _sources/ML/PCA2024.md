...
The first step is to load the dataset into memory, this can be done with Pandas as we saw into the <a href="{{ site.baseurl }}{% link _teaching/introduction_to_python_numpy2024.md %}">introductory post</a>.
...
Following the <a href="{{ site.baseurl }}{% link _teaching/short_introduction_to_ML2024.md %}">introductory post on Machine Learning</a>, first of all we convert the dataframe into a matrix with `numpy`, and then we split the input matrix $$X$$ and the corresponding target vector $$Y$$. Finally, we note that $$X$$ is an $$N \times d$$ matrix, where $$N = 42000$$ and $$d = 784$$. Since in our notations the shape of $$X$$ **must** be $$d \times N$$, we have to transpose it. 
...
Before implementing the algorithm performing PCA, you are required to split the dataset into a training set and a test set, following the <a href="{{ site.baseurl }}{% link _teaching/short_introduction_to_ML2024.md %}">introductory post on Machine Learning</a>. Remember that to correctly split $$X$$, it has to be random. 
